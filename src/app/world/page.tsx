    ...
    <main style={page}>
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
        ...
        <span className="wl-num" style={{ fontSize: 11, color: "var(--text-secondary)" }}>
          TICK {state.tick} · {Object.keys(state.players).length} CMDR
        </span>
      </div>
      {error && (
        <div style={{ marginTop: 8 }}>
          <Badge tone="blood" variant="soft" icon="⚠">{error}</Badge>
        </div>
      )}
      ...
      {myBase ? (
        <Panel label="HEADQUARTERS" accent padding="10px 14px" style={{ marginTop: 12 }}
          ...
          <div style={{ display: "flex", gap: 20, flexWrap: "wrap", alignItems: "center" }}>
            ...
            <Stat label="TERRITORY" value={`${myBase.ownedHexes.length} HEX`} accent="neutral" />
          </div>
        </Panel>
      ) : (
      ...
      <div style={mapWrap}>
        ...
        <svg viewBox="-260 -260 520 520" style={{ width: "100%", height: "100%", display: "block", position: "relative" }}>
          {Object.values(state.hexes).map((h) => {
            ...
            const rim = selected ? "var(--rim-selected)" : mine ? "var(--rim-owned)" : owner ? "var(--rim-enemy)" : "var(--rim-neutral)";
            return (
              <g key={key} transform={`translate(${x},${y})`} onClick={() => onHexClick(key, h.q, h.r)}
                style={{ cursor: "pointer", filter: selected ? "drop-shadow(0 0 4px rgba(245,179,1,0.6))" : undefined }}>
      ...
      {myBase && sel && (
        ...
        >
          {selBuilding ? (
            ...
            <div>
              <span className="wl-label">CONSTRUCT</span>
              <div style={{ display: "grid", gap: 6, marginTop: 8 }}>
                {buildable.map((id) => {
                  ...
                  return (
                    <Button key={id} variant="secondary" full disabled={!ok} style={rowBtn}
  ...
  return (
    <div style={{ fontSize: 13 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
        ...
        {building.buffer != null && building.level >= 1 && (
  ...
  for (const owned of base.ownedHexes) {
    ...
    for (const [dq, dr] of dirs) {
      ...
      if (state.hexes[k] && !state.claimedHexes[k]) {
        return (
          <Button variant="outline" full icon="➕" style={{ marginTop: 10, justifyContent: "flex-start" }}
            onClick={() => onExpand(q, r)}>
...
const page: CSSProperties = {
  ...
  padding: "max(16px, env(safe-area-inset-top)) 16px 16px", fontFamily: "var(--font-ui)",
};
